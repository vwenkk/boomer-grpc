...
// SetAutoStart auto startSpawning to load testing
func (b *Boomer) SetAutoStart() {
	b.masterRunner.autoStart = true
}

// RunMaster startSpawning to run master runner
func (b *Boomer) RunMaster() {
	b.masterRunner.run()
}

// RunWorker startSpawning to run worker runner
...
// EnableCPUProfile will startSpawning cpu profiling after run.
...
// EnableMemoryProfile will startSpawning memory profiling after run.
...
func (b *Boomer) Run(tasks ...*Task) {
	if b.cpuProfile != "" {
		...
		if err != nil {
			log.Error().Err(err).Msg("failed to startSpawning cpu profiling")
		}
	}
	if b.memoryProfile != "" {
		err := startMemoryProfile(b.memoryProfile, b.memoryProfileDuration)
		if err != nil {
			log.Error().Err(err).Msg("failed to startSpawning memory profiling")
		}
	}
	...
	case DistributedWorkerMode:
		...
		b.workerRunner.initTask = b.initTask
...
func (b *Boomer) RecordSuccess(requestType, name string, responseTime int64, responseLength int64) {
	if b.workerRunner == nil && b.localRunner == nil && b.masterRunner == nil {
		return
	}
	var runnerStats *requestStats
	switch b.mode {
...
func (b *Boomer) ReBalance(Args *Profile) error {
	if !b.masterRunner.isStarting() {
		return errors.New("no startSpawning")
	}
	if int(Args.SpawnCount) < b.masterRunner.server.getAvailableClientsLength() {
		return errors.New("spawn count should be greater than available worker count")
	}
	...
	for _, task := range tasks {
		if task.Name == "" {
			...
			for _, name := range taskNames {
				if name == task.Name {
					log.Info().Msg("Running " + task.Name)
					task.Fn(ctx)
				}
			}
		}
...
func Run(tasks ...*Task) {
	...
	defaultBoomer.SetRateLimiter(maxRPS, requestIncreaseRate)
	defaultBoomer.masterHost = masterHost
	defaultBoomer.masterPort = masterPort
	defaultBoomer.workerRunner = newWorkerRunner(masterHost, masterPort)
	defaultBoomer.SetMode(DistributedWorkerMode)
	defaultBoomer.AddOutput(NewConsoleOutput())
	...
	defaultBoomer.EnableGracefulQuit(context.Background())
