...
type runner struct {
	...
	// done is closed when all goroutines from startSpawning() complete.
	doneChan chan bool
...
// gracefulStop should be called after a startSpawning(), otherwise it will block forever.
...
func (r *workerRunner) run() {
	...
	r.clientCond.Signal() // 通知runtask client 准备好了
...
func (r *workerRunner) startSpawning() {
...
func (r *masterRunner) clientListener() {
	log.Info().Msg("clientListener, startSpawning to deal message from worker")
	for {
		select {
		case <-r.closeChan:
			return
		case msg := <-r.server.recvChannel():
			...
			go func() {
				...
				case typeQuit:
					...
					// 统计退出client数量，如果等于expectWorkers 退出
					if r.server.getWorkersLengthByState(StateQuitting) == r.expectWorkers {
						r.close()
					}
...
func (r *masterRunner) run() {
	...
	// startSpawning grpc server
	err := r.server.start()
	if err != nil {
		log.Error().Err(err).Msg("failed to startSpawning grpc server")
		return
	}
	...
	if r.autoStart {
		go func() {
			log.Info().Msg("auto startSpawning, waiting expected workers joined")
			ticker := time.NewTicker(1 * time.Second)
